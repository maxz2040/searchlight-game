...
//
// iPad Safari: calls unlockAudio() synchronously on the button click so
// the Web Audio context is created and resumed inside a user gesture.
// Without this, playPing() later (called from a rAF loop) would fail
// silently on iOS because AudioContext.resume() is blocked outside a
// user gesture event handler.
...
export function Tutorial() {
  ...
  function handleBegin() {
    // Unlock Web Audio inside this synchronous user-gesture handler so
    // subsequent playPing() calls from the rAF collision loop succeed on
    // iOS Safari. Must happen before beginPlaying() schedules any React
    // updates that could flush asynchronously.
    unlockAudio();
    begin();
  }
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center gap-7 px-6 text-center">
        ...
        >
          Drag your finger across the screen. Move the lantern&rsquo;s light
